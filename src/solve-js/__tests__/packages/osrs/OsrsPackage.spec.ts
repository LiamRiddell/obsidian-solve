...
import { OsrsItemTrie } from "@solve-js/packages/osrs/OsrsItemTrie";
import { osrsItemNormalizerRule, GAME_ITEM_TYPE } from "@solve-js/packages/osrs/OsrsItemNormalizer";
import { GameItemParselet, OsrsKeywordParselet, OSRS_PLUGIN_FN_IDX } from "@solve-js/packages/osrs/OsrsParselet";
import { registerOsrsPluginFunction, unregisterOsrsPluginFunction, setOsrsQueryClient } from "@solve-js/packages/osrs/OsrsVmHandler";
import { uomValue } from "@solve-js/vm/Value";
import { OSRS_ITEMS, OSRS_ITEM_NAME_TO_ID, osrsItemQueryKey } from "@solve-js/packages/osrs/OsrsItemVocabulary";
...
import { osrsLexerPlugin } from "@solve-js/packages/osrs/OsrsLexerPlugin";
import type { OsrsItem } from "@solve-js/packages/osrs/types";
...
describe("OSRS keyword prefix path (end-to-end)", () => {
  function createEngine(): {
    ...
    lexer.registerPlugin(osrsLexerPlugin);
  ...
  test("parses 'ge Dragon Hide' through full pipeline (ge alias for osrs keyword)", () => {
    ...
    // Step 1: Lex — "ge" is also mapped to OSRS_KEYWORD by osrsLexerPlugin
