/**
 * SolveLanguageService Benchmarks - Jest compatible
 *
...
describe("SolveLanguageService Benchmarks", () => {
  ...
  for (const c of highlightCases) {
    test(`getSemanticTokens cold: "${c.name}" (${c.iters.toLocaleString()} iter)`, () => {
      ...
      const service = new SolveLanguageService(engine);
      let line = 1;
      const r = benchmarkFn(() => {
  ...
  test("getCompletions warm with a large document-variable pool (500 variables)", () => {
    ...
    const service = new SolveLanguageService(engine, { variableNameSource: () => varNames });
