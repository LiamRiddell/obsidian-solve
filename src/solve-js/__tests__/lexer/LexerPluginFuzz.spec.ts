/**
 * Property-based (fuzz) tests for LexerPlugin registration.
 *
...
function tokenize(input: string, plugin?: LexerPlugin): Array<{ type: string; value: string }> {
...
function types(input: string, plugin?: LexerPlugin): string[] {
  return tokenize(input, plugin).map((t) => t.type);
}

function values(input: string, plugin?: LexerPlugin): string[] {
...
describe("LexerPlugin Fuzz — keyword collision resistance", () => {
  test("registering a built-in keyword as plugin keyword throws SolveError", () => {
  ...
  test("registering uppercase variant of built-in keyword as plugin keyword throws SolveError", () => {
  ...
  test("plugin keywords that look like function names are blocked by collision guard", () => {
    ...
    for (const kw of ["max", "round", "sin", "sqrt", "abs"]) {
      expect(() => new ExpressionLexer("en").registerPlugin({
    ...
    expect(() => new ExpressionLexer("en").registerPlugin(plugin)).not.toThrow();
...
describe("LexerPlugin Fuzz — operator collision resistance", () => {
  test("registering a built-in two-char operator as plugin operator throws SolveError", () => {
...
describe("LexerPlugin Fuzz — unit collision resistance", () => {
  test("registering a built-in unit as plugin unit throws SolveError", () => {
...
describe("LexerPlugin Fuzz — mixed expression collisions", () => {
...
describe("LexerPlugin Fuzz — stress test with all fuzz cases", () => {
  // Generate a large, noisy plugin with many entries
  const STRESS_PLUGIN: LexerPlugin = {
    keywords: {
...
describe("LexerPlugin Fuzz — edge cases and boundary conditions", () => {
  ...
  test("plugin registrations are isolated per ExpressionLexer instance", () => {
    ...
    lexerA.registerPlugin({
      keywords: { only_a: "ONLY_A" },
    });

    lexerB.registerPlugin({
...
describe("LexerPlugin Fuzz — safety boundaries", () => {
...
describe("LexerPlugin Fuzz — unregisterPlugin", () => {
  test("unregisterPlugin removes plugin keywords", () => {
    const lexer = new ExpressionLexer("en");
    const plugin: LexerPlugin = {
  ...
  test("unregisterPlugin removes plugin operators", () => {
    const lexer = new ExpressionLexer("en");
    const plugin: LexerPlugin = {
  ...
  test("unregisterPlugin with never-registered plugin is safe (no-op)", () => {
    const lexer = new ExpressionLexer("en");
    const plugin: LexerPlugin = {
      keywords: { my_key: "MY_KEY" },
    };

    expect(() => lexer.unregisterPlugin(plugin)).not.toThrow();
  ...
  test("unregisterPlugin only removes its own entries, not other plugins'", () => {
    const lexer = new ExpressionLexer("en");
    const pluginA: LexerPlugin = {
      keywords: { key_a: "KEY_A" },
      units: ["unit_a"],
    };
    const pluginB: LexerPlugin = {
      keywords: { key_b: "KEY_B" },
      units: ["unit_b"],
    };

    lexer.registerPlugin(pluginA);
    lexer.registerPlugin(pluginB);
    ...
    lexer.unregisterPlugin(pluginA);
  ...
  test("re-register after unregisterPlugin works correctly", () => {
    const lexer = new ExpressionLexer("en");
    const plugin: LexerPlugin = {
    ...
    const pluginV2: LexerPlugin = {
      keywords: { my_key: "MY_KEY_V2" },
    };
    lexer.registerPlugin(pluginV2);
    lexer.reset("my_key");
...
describe("LexerPlugin Fuzz — collision guard edge cases", () => {
  test("mixed plugin with conflicting and non-conflicting entries throws on first conflict", () => {
    ...
    const lexer = new ExpressionLexer("en");
    expect(() => lexer.registerPlugin(plugin))
      .toThrow(/conflicts with built-in keyword/i);
  ...
  test("registerPlugin with no conflicting entries does not throw", () => {
    const plugin: LexerPlugin = {
      keywords: { my_key: "MY_KEY" },
      operators: { "->": "ARROW" },
      units: ["myunit"],
    ...
    const lexer = new ExpressionLexer("en");
    expect(() => lexer.registerPlugin(plugin)).not.toThrow();
