...
export class ExponentVisitor implements IGenericResultVisitor<INumericResult> {
	...
	private float(left: FloatResult, right: INumericResult) {
		...
		return new FloatResult(Math.pow(left.value, coercedRight.value));
	}
	...
	private integer(left: IntegerResult, right: INumericResult) {
		...
		return new IntegerResult(Math.pow(left.value, coercedRight.value));
	}
	...
	private percentage(left: PercentageResult, right: INumericResult) {
		if (right instanceof PercentageResult) {
			return new FloatResult(
				Math.pow(left.value / 100, right.value) / 100
			);
		}
