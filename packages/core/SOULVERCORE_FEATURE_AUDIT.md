...
| `totals-and-subtotals` | ⛔ / ⏸️ | Mostly a floating-total UI widget + `⌘T` subtotal marker — app-layer. One real engine-adjacent piece — "a subtotal sums every line above back to the last subtotal/heading" — is a genuine cross-line aggregation capability `ThreeTierEvaluator`/`DocumentModel` don't expose today. Deferred: would need a new "give me every result since the last marker" primitive, a bigger architectural change than a package addition. Tracked as a real follow-up, not attempted. |
| `line-references` | ⛔ | Drag/double-click/`⌘\` to insert a reference to the previous line's answer — confirmed to be a pure editor gesture with no distinct textual syntax. If ever built, the natural engine primitive is a `prev`/`ans` keyword resolving to the immediately-preceding line's cached result — a small future `src/app` + package pairing, not in scope here. |
...
| `live-data/knowledge-assistant` | ✅ (opt-in) | `packages/knowledge/` — `createKnowledgePackage({ answerQuery })`, opt-in for the same reason as stocks. `<query text> = ?` via a new `rawLinePatterns` lexer extension point (see `ARCHITECTURE.md` §5.1) since the query text isn't valid Solve syntax. |
